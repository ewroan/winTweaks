...
echo https://sites.google.com/view/melodystweaks/basictweaks

echo #####
echo Avoid the use of uncontiguous portions of low-memory from the OS. Boosts memory performance and improves microstuttering at least 80% of the cases. 
echo Also fixes the command buffer stutter after disabling 5-level paging on 10th gen Intel. Causes system freeze on unstable memory sticks.
...
echo #####
echo Disable DMA memory protection and cores isolation ("virtualization-based protection").
bcdedit /set vsmlaunchtype Off
bcdedit /set vm No

echo #####
echo Disable Process and Kernel Mitigations.
powershell "ForEach($v in (Get-Command -Name \"Set-ProcessMitigation\").Parameters[\"Disable\"].Attributes.ValidValues){Set-ProcessMitigation -System -Disable $v.ToString() -ErrorAction SilentlyContinue}"
powershell "Remove-Item -Path \"HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\*\" -Recurse -ErrorAction SilentlyContinue"

echo #####
echo Use realtime priority for csrss.exe.
...
echo #####
echo Use big system memory caching to improve microstuttering.
...
echo #####
echo Disable synthetic TSC tick and use accurate RTC instead (not to be confused with useplatformclock).
echo Enable HPET in BIOS for best results. Only for untweaked systems (TSC recommended instead on tweaked systems).
...
echo #####
echo Set a reliable 1 ms (minimum) timestamp. Only for untweaked systems (disabling it with 0 is recommended on tweaked systems).
...
echo #####
echo Force contiguous memory allocation in the NVIDIA driver.
echo (0000 may vary depending on the GPU number)
...
echo #####
echo Enable UDP offloading.
netsh int udp set global uro=enabled

echo #####
echo Enable Teredo and 6to4 (Win 2004 Xbox LIVE fix).
netsh int teredo set state natawareclient
netsh int 6to4 set state state=enabled

echo #####
echo Enable Winsock Send Autotuning (dynamic send-buffer)
netsh winsock set autotuning on

echo #####
echo Decrease mouse and keyboard buffer sizes.
...
echo #####
echo Tell Windows to stop tolerating high DPC/ISR latencies.
...
echo #####
echo Disable Windows Defender Antivirus
...
echo #####
echo https://sites.google.com/view/melodystweaks/windows-11-basic-optimizer

echo #####
echo Process Mitigations
...
echo #####
echo Per-process Process Mitigations
...
echo #####
echo keep MsSense alone because it causes problems
reg delete "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\MsSense.exe" /v "MitigationOptions" /f
...
echo #####
echo the one below is actually 0 to disable customer improvement program, idk why
...
echo #####
echo Defender and SmartScreen
echo we cannot disable Defender in Win11 but at least make it slimmer and stop it from taking actions on its own
echo new - manually decrease Defender priority (since the Policy does not work anymore and is completely ignored)
...
echo #####
echo Hardware accelerated scheduling
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v "HwSchMode" /t REG_DWORD /d 2 /f

echo #####
echo use bbr2 (BBR V2) as a congestion control provider
netsh int tcp set supplemental internet congestionprovider=bbr2

echo #####
echo use the weak host model by default (same as Linux)
...
echo Get Insider Updates without joining the Insider Program and without having Telemetry enabled
echo reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" /v "BranchReadinessLevel" /t REG_DWORD /d 2 /f
echo reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" /v "ManagePreviewBuilds" /t REG_DWORD /d 1 /f
echo reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" /v "ManagePreviewBuildsPolicyValue" /t REG_DWORD /d 2 /f
